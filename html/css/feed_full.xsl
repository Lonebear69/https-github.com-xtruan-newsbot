...
<xsl:variable name="date" select="/rss/newsbot/date"/>
<xsl:variable name="runtime" select="/rss/newsbot/runtime"/>		
<xsl:template match="/">
<html>
  <head>
  ...
  <body>
    <div id="explanation">
      ...
      <p>Newsbot generated this snapshot at <xsl:value-of select="$date"/>. It took <xsl:value-of select="$runtime"/> second(s).</p>
    </div>
